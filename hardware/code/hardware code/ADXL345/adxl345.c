...
#define ADXL345_RANGE	4.0f							// 当前量程: 4.0g
#define ADXL345_SCALE	( ADXL345_RANGE / 32768.0f )	//原始值转化为 g 的转化系数
...
void adxl345_init()
{
		adxl345_write_reg(0X31,0X09);		//低电平中断输出,13位全分辨率,输出数据右对齐,4g量程 
...
{
	float  ax_raw, ay_raw, az_raw;	// ADXL345原始数据(16位有符号数)
	float  ax, ay, az;				// 转化后的实际加速度(单位：g)
		
	adxl345_read_average(&ax_raw, &ay_raw, &az_raw, 10);	//读取ADXL345原始数据的平均值(10次平均值)
	...
	//计算姿态角
	*x_angle = atan(ax / sqrt(az*az + ay*ay))*180.0f / M_PI;	//X轴夹角
	*y_angle = atan(ay / sqrt(ax*ax + az*az))*180.0f / M_PI;	//Y轴夹角
	*z_angle = atan(sqrt(ax*ax + ay*ay) /az) *180.0f / M_PI;	//Z轴夹角
} 
